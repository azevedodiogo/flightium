...
As consultas sobre a base de dados são disponibilizadas através de um motor de interrogações otimizado que executa seis tipos de queries. Todas as queries suportam uma variação de sintaxe onde o caracter S após o identificador altera o delimitador padrão de ponto e vírgula para o símbolo de igualdade no ficheiro de saída. Os parâmetros e resultados de cada query encontram-se resumidos na tabela abaixo:

| Query | Entrada (Input) | Saída (Output) | Descrição |
| --- | --- | --- | --- |
| Query 1 | Código IATA de um aeroporto | Informações e volume de voos | Retorna dados de localização (nome, cidade, país e tipo) juntamente com o volume acumulado de voos de partida e chegada. |
| Query 2 | Número N e opcionalmente fabricante | Lista ordenada de aeronaves mais ativas | Devolve as N aeronaves com maior registo de voos efetuados (opcionalmente filtradas por um fabricante específico). |
| Query 3 | Intervalo de datas YYYY-MM-DD YYYY-MM-DD | Aeroporto mais visitado no período | Encontra o aeroporto que obteve o maior número de visitas totais (chegadas + partidas) durante o intervalo de tempo dado. |
| Query 4 | Intervalo de datas YYYY-MM-DD YYYY-MM-DD | Passageiro com maior despesa | Identifica o utilizador que gastou a quantia mais elevada no acumulado de voos e reservas no período correspondente. |
| Query 5 | Número N | N companhias com maiores atrasos | Lista as N companhias aéreas com o maior tempo de atraso médio nos seus voos, ordenadas de forma decrescente. |
| Query 6 | Nacionalidade de passageiros | Aeroporto mais visitado por nacionalidade | Determina qual o aeroporto que recebeu o maior número de voos onde viajaram passageiros da nacionalidade informada. |
...
Queries on the database are made available through an optimized query engine that runs six core queries. All queries support a syntax variant where an S character after the query number changes the standard semicolon separator to an equals separator in the output files. The parameters and results of each query are summarized in the table below:

| Query | Input | Output | Description |
| --- | --- | --- | --- |
| Query 1 | IATA code of an airport | Airport details and flight traffic | Returns localization details (name, city, country, type) alongside the total accumulated volume of departure and arrival flights. |
| Query 2 | Integer N and optionally a manufacturer | Ordered list of most active aircraft | Returns the top N aircraft with the highest count of registered flights (optionally filtered by a specific manufacturer). |
| Query 3 | Date range YYYY-MM-DD YYYY-MM-DD | Most visited airport in date range | Finds the airport with the highest total visits (arrivals + departures) within the specified date range. |
| Query 4 | Date range YYYY-MM-DD YYYY-MM-DD | Highest spending passenger | Determines the passenger who spent the most overall on flights and reservations during the given time window. |
| Query 5 | Integer N | N airlines with highest average delays | Lists the top N airlines with the largest average flight delays, sorted in descending order. |
| Query 6 | Passenger nationality | Most visited airport by nationality | Calculates which airport received the most flights carrying passengers of the specified nationality. |
